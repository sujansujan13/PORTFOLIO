...
interface TimelineFormHeaderProps {
  ...
  onDiscard?: () => void;
  onSaveDraft?: () => void; // Optional handler for saving drafts
}
...
export function TimelineFormHeader({
  ...
  // Determine button click action and text based on mode
  const secondaryAction = isEditing ? onDiscard : onSaveDraft;
  const secondaryLabel = isEditing ? "Discard Changes" : "Save as Draft";
  ...
  return (
    <div className="space-y-4 mb-6">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-primary-foreground bg-primary hover:bg-primary/90 rounded-md shadow-sm transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
          >
            {isSubmitting ? (
      ...
      <div>
        ...
        >
          {isEditing ? "Edit Timeline Entry" : "Add Experience Entry"}
        </motion.h1>
